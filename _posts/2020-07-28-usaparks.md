...
<br><br>The inverse document frequency is the logarithmically scaled inverse fraction of the documents that contain the word (obtained by dividing the total number of documents by the number of documents containing the term, and then taking the logarithm of that quotient):<br><br>
$\mathrm{idf}(t, D) = \log \frac{N}{|\{d \in D: t \in d\}|}$
<br>
where
&nbsp;&nbsp;$$N$$: total number of documents in the corpus $$N = {|D|}$$ <br>&nbsp;$$ |\{d \in D: t \in d\}| $$ : number of documents where the term $$t$$ appears (i.e., $$ \mathrm{tf}(t,d) \neq 0$$). If the term is not in the corpus, this will lead to a division-by-zero. It is therefore common to adjust the denominator to $$1 + |\{d \in D: t \in d\}|$$.
<br><br>Then tf–idf is calculated as:<br><br>
